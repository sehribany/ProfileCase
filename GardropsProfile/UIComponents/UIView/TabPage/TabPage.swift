...
@IBDesignable public class TabPage: UIView {
    
    public var isIconMode: Bool? { didSet { createButtons() } }

    public var isLockIconMode: Bool? { didSet { createButtons() }}
    
    public var isFirsIcontHidden: Bool? = nil
     
    public var isSecondIconHidden: Bool? = nil
    
    @IBInspectable public var selectedTextColor: UIColor = .appPink1{ didSet { createButtons() } }

    @IBInspectable public var selectedBackgroundColor: UIColor = .appWhite { didSet { createButtons() } }
    ...
    @IBInspectable public var unselectedBackgroundColor: UIColor = .appWhite { didSet { createButtons() } }
    ...
    @IBInspectable public var unselectedBorderColor: UIColor = .appPink1 { didSet { createButtons() } }
    
    @IBInspectable public var unselectedTextFont: UIFont? { didSet { createButtons() } }
    
    var selectedIconColor: UIColor? { didSet { createButtons() } }
    
    var unselectedIconColor: UIColor? { didSet { createButtons() } }
    ...
    public func setWithControllers(data: [(title: String, controller: UIViewController)], on scrollView: UIScrollView, completion: @escaping SegmentedViewHandler) {
        ...
        bottomLineView.backgroundColor = .appPink2
        ...
        bottomLineView.widthAnchor.constraint(equalTo: widthAnchor, multiplier: 1).isActive = true
    }
    ...
    var selectedIndex = 0 {
        willSet {
            ...
            if let button = segmentButtons?[newValue] {
                button.setButton(isEnabled: false)
        ...
        didSet {
            ...
            if let previewButton = segmentButtons?[oldValue] {
                previewButton.setButton(isEnabled: true)
...
extension TabPage {
    ...
    private func createButtons() {
        ...
        for i in 0 ..< itemTitles.count {
            ...
            button.isBottomSheetButton = self.isBottomSheet
            addSubview(button)
            ...
            if isBottomSheet ?? false {
                button.unselectedBackgroundColor = .appWhite
            } else {
            ...
            if isIconMode ?? false {
                button.setImage(UIImage(named: itemTitles[i])?.withRenderingMode(.alwaysTemplate), for: .normal)
                button.selectedIconColor = selectedIconColor
                button.unSelectedIconColor = unselectedIconColor
                button.imageEdgeInsets = UIEdgeInsets(top: 5, left: 0, bottom: 7, right: 0)
                button.contentMode = .scaleAspectFit
                button.imageView?.contentMode = .scaleAspectFit
                
            } else if isLockIconMode ?? false {
                button.setTitle(itemTitles[i], for: .normal)
                button.selectedIconColor   = .appWhite
                button.unSelectedIconColor = .appWhite
    ...
    @objc func appNavigationButtonAction(_ sender: UIButton) {
...
extension TabPage: UICollectionViewDelegate,UICollectionViewDelegateFlowLayout {
